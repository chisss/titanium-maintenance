...
[系统全景](#titanium-是什么) · [服务索引](#微服务索引) · [架构边界](#架构与领域边界) · [当前服务](#titanium-maintenance) · [快速开始](#快速开始)
...
> **保全域**：管理保单生效后的变更案件，包括配置冻结、审核、核保、算费、结算和合同生效。

| 属性 | 内容 |
|---|---|
| 限界上下文 | 保全域（核心域） |
| 核心模型 | MaintenanceCase、MaintenanceItem、WorkflowTask |
| 主要上游 | Policy、Customer、Product |
| 主要下游 | Underwriting、Billing、Payment |
| 默认地址 | [`http://localhost:8083/maintenance`](http://localhost:8083/maintenance) |
| GitHub | [`titanium-maintenance`](https://github.com/chisss/titanium-maintenance) |

### 能力与边界

| 本服务负责 | 本服务不负责 |
|---|---|
| 保全案件和项目状态机<br/>字段变更、审核与核保证据<br/>保全报价和资金双门禁<br/>立即、未来及追溯生效编排 | 保单权威合同状态<br/>产品定价公式<br/>账务与支付最终状态 |
...
以下截图来自 [Titanium 管理后台](https://github.com/chisss/titanium-admin-web)，仅展示保全域对应的业务页面。截图包含可核验的示例数据，并按页面上下排列，GitHub 页面可直接加载。

<p align="center">
  <img src="./docs/screenshots/titanium-maintenance-list-20260828.png" alt="保全工单列表：案件状态与生效状态" width="100%" />
</p>

<p align="center"><sub>保全工单 · 保单号、保全项、来源及案件状态</sub></p>

<p align="center">
  <img src="./docs/screenshots/titanium-maintenance-workbench-20260828.png" alt="保全工作台：字段变更与流程任务" width="100%" />
</p>

<p align="center"><sub>保全工作台 · 字段变更、流程任务、快照及生效回执</sub></p>

### 协作关系

```mermaid
flowchart LR
    UP[Policy、Customer、Product] -->|API / Event| CURRENT[保全域]
    CURRENT -->|API / Event| DOWN[Underwriting、Billing、Payment]
```
...
| 模块 | 职责 |
|---|---|
| `titanium-maintenance-common` | 通用层：模块内枚举、异常和常量 |
| `titanium-maintenance-api` | API 层：服务间契约、Feign 接口和 DTO |
| `titanium-maintenance-domain` | 领域层：聚合、值对象、领域事件及 Port |
| `titanium-maintenance-application` | 应用层：用例编排、命令与查询协调 |
| `titanium-maintenance-infrastructure` | 基础设施层：Repository、远程 Adapter、消息与持久化 |
| `titanium-maintenance-query` | 查询层：CQRS 读模型与查询处理器 |
| `titanium-maintenance-web` | Web 层：REST 入口、请求校验和响应装配 |
| `titanium-maintenance-bootstrap` | 启动层：Spring Boot 入口、配置和 Liquibase |
...
```bash
git clone https://github.com/chisss/titanium-maintenance.git
...
```bash
mvn -pl titanium-maintenance-bootstrap -am spring-boot:run
```

默认访问地址为 `http://localhost:8083/maintenance`。数据库、Redis、Kafka、下游服务地址及环境变量以 `titanium-maintenance-bootstrap/src/main/resources/application.yml` 为准。
