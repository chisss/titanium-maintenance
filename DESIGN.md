...
- Product Offering 远程调用使用固定服务名和固定路径，不接受调用方 URL；Adapter 结构化解析错误响应并校验租户、产品、版本、摘要和保全项编码。当前建案路由仍信任网关注入的租户/操作人头；案件详情敏感查看只信任 Spring Security 认证主体权限，不信任请求头自报权限。
...
- 人工来源不能完成业务校验或提交条件结论；系统 API 在真实服务认证接入前属于部署信任边界，不应暴露到普通用户网络。
