...
public class MaintenanceServiceImpl implements MaintenanceService {
    ...
    public List<Maintenance> findPendingMaintenancesByPolicyId(PolicyId policyId) {
        return maintenanceRepository.findByPolicyId(policyId).stream()
                .filter(m -> m.getStatus() == MaintenanceStatus.PENDING || 
                             m.getStatus() == MaintenanceStatus.PROCESSING || 
                             m.getStatus() == MaintenanceStatus.APPROVED)
    ...
    public boolean isPolicyUnderMaintenance(PolicyId policyId) {
        ...
        return maintenances.stream()
                .anyMatch(m -> m.getStatus() == MaintenanceStatus.PROCESSING || m.getStatus() == MaintenanceStatus.PENDING);
    }
    ...
    public void deleteMaintenance(Maintenance maintenance) {
        if (maintenance.getStatus() != MaintenanceStatus.PENDING) {
            throw new MaintenanceStatusException(maintenance.getId().getId(),
                    maintenance.getStatus().name(), "DELETED", "仅待处理状态的保全允许删除");
        }
